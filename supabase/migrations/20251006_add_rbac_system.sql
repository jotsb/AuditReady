...
-- Set created_by for existing businesses to their user_id (assuming one-to-one relationship)
UPDATE businesses
...
SELECT id, user_id, 'owner'::business_role_type, created_at
FROM businesses
WHERE NOT EXISTS (
...
-- Set uploaded_by for existing receipts to the business owner
UPDATE receipts r
SET uploaded_by = b.user_id
FROM businesses b
WHERE r.business_id = b.id AND r.uploaded_by IS NULL;
...
CREATE POLICY "Business members can view approvals in their business"
  ...
  USING (
    EXISTS (
      ...
      AND is_business_member(auth.uid(), r.business_id)
    )
  );
...
CREATE POLICY "Owners and managers can update approvals"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM receipts r
      WHERE r.id = receipt_approvals.receipt_id
      AND is_business_owner_or_manager(auth.uid(), r.business_id)
    )
  );
...
DROP POLICY IF EXISTS "System admins can view all categories" ON categories;
CREATE POLICY "System admins can view all categories"
  ON categories FOR SELECT
  TO authenticated
  USING (is_system_admin(auth.uid()));

DROP POLICY IF EXISTS "Business members can view categories in their business" ON categories;
CREATE POLICY "Business members can view categories in their business"
  ON categories FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Owners and managers can create categories" ON categories;
...
DROP POLICY IF EXISTS "Owners and managers can update categories" ON categories;
...
DROP POLICY IF EXISTS "Owners and managers can delete categories" ON categories;
...
BEGIN
  INSERT INTO business_members (business_id, user_id, role, joined_at)
  VALUES (NEW.id, NEW.user_id, 'owner'::business_role_type, NEW.created_at);
  RETURN NEW;
END;
